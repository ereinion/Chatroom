...
def connect():
    ...
    query = cur.mogrify("select name, text from chatlog WHERE room = '%s' ORDER BY id DESC LIMIT 10;" % (newRoom))
    cur.execute(query)
...
def message(text):
    ...
    cur.execute(query)
    conn.commit()
    newmessage = {'text': text, 'name': user}
...
def searching(searchQuery):
    ...
    for m in searchedmessages:
        retex = m[1]
        renam = m[0]
        returnmessages={'text': retex, 'name': renam}
        #print(returnmessages)
        emit('searched', returnmessages, room=room)
